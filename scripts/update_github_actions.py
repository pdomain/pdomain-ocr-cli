...
def run_gh(command: list[str]) -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run(  # noqa: S603  # executable is resolved to an absolute path; argument list avoids shell parsing
...
def update_github_actions(
    *,
    ...
    updated here. That field is a contributor floor (e.g. >=0.11.16) and must
    not auto-track the latest uv release. Auto-tracking caused the dep-refresh
    job to self-poison: uv rewrites the pin to the latest exact version, then
    immediately violates the new constraint because the running uv process is
    the older version that setup-uv installed at job start.
    """
